...
function getElement(selector, callback, timeout = 1000, maxretries = 5, attempt = 1){

    if(attempt >= maxretries){
        throw Error(`try to get element with selector '${selector}' but the max retries limit reached (${maxretries}).`);
    }
...
function createResizeBadge() {
    ...
    badge.id = 'ayvh-badge-resize';
    badge.classList.add('ayvh-badge-resize');
...
function placeBadge(badge, anchor){
    ...
    const {top: anchorTop, left: anchorLeft, width: anchorWidth, height: anchorHeight} = anchorRect;
...
getElement(youtubeVideoContainerSelector, (videoContainer) => {
    ...
    const theaterContainer = document.querySelector(youtubeTheaterContainerSelector);
    ...
    badge.addEventListener('mousedown', (event) => {
        ...
        document.body.classList.add('ayvh-disable-user-select');
    ...
    window.addEventListener('mouseup', (event) => {
        ...
        if(mouseIsPressed){
            ...
            document.body.classList.remove('ayvh-disable-user-select');
    ...
    window.addEventListener('mousemove', (event) => {
        ...
        if(mouseIsPressed) {
            ...
            const {width: videoTagWidth, height: videoTagHeight} = videoTagRect;
            ...
            videoTag.style.left = ( window.innerWidth - ( (savedVideoHeight + dy ) * videoRatio ) ) / 2 + 'px';
        }
    });
});